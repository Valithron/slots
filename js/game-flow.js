(() => {
  ...
  function isLongCelebrationTier(tier) {
    return tier === "nice" || tier === "big" || tier === "jackpot";
  }
  ...
  function getCountUpDuration(tier, { reducedMotion = false } = {}) {
    ...
    const bounded = Math.min(
      CONFIG.winTiers.countUpMaximum,
      Math.max(CONFIG.winTiers.countUpMinimum, configured),
    );
  ...
  function getCountUpValue(totalWin, progress) {
    ...
    return Math.floor(totalWin * eased);
  }
