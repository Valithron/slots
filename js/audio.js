(() => {
  ...
  function createAudio(getSoundEnabled) {
    ...
    function playTick(pitch = 1) {
      tone({ frequency: 170 * pitch, duration: 0.028, type: "square", gain: 0.018 });
    }
    ...
    function playTierSound(tier) {
      if (tier === "small") {
        chord([740, 930, 1110], { spacing: 0.055, duration: 0.13, gain: 0.045 });
        return;
      }
      if (tier === "nice") {
        chord([523, 659, 784, 1047], { spacing: 0.1, duration: 0.28, gain: 0.07 });
        tone({ frequency: 196, endFrequency: 392, duration: 0.62, type: "sine", gain: 0.04 });
        return;
      }
      if (tier === "big") {
        chord([392, 523, 659, 784, 1047], { spacing: 0.12, duration: 0.34, gain: 0.085 });
        tone({ frequency: 98, endFrequency: 196, duration: 0.86, type: "sawtooth", gain: 0.035 });
        return;
      }
      if (tier === "jackpot") {
        chord([262, 330, 392, 523, 659, 784, 1047, 1319], { spacing: 0.1, duration: 0.38, gain: 0.08 });
        chord([523, 659, 784, 1047], { spacing: 0.08, duration: 0.5, gain: 0.06, type: "sine" });
        tone({ frequency: 65, endFrequency: 260, duration: 1.4, type: "sawtooth", gain: 0.04 });
      }
    }

    function playWinSound(amount) {
      playTierSound(amount >= 100 ? "nice" : "small");
    }

    function playLossSound() {
      tone({ frequency: 180, endFrequency: 120, duration: 0.22, type: "triangle", gain: 0.035 });
    }

    function playErrorSound() {
      tone({ frequency: 115, duration: 0.12, type: "square", gain: 0.035 });
      tone({ frequency: 92, duration: 0.14, type: "square", gain: 0.03, when: 0.13 });
    }

    function playButtonTone() {
      tone({ frequency: 480, duration: 0.055, type: "sine", gain: 0.04 });
    }

    function playRefillSound() {
      chord([330, 440, 550, 660], { spacing: 0.07, duration: 0.12, gain: 0.05 });
    }
